<?php
/**
 * Template part for displaying page content in page.php.
 *
 * @link https://codex.wordpress.org/Template_Hierarchy
 *
 * @package lorainccc
 */

//Get custom fields
global $post;

$post_id = $post->ID;

$last_name = get_post_meta( $post_id, 'lc_fac_staff_dir_lastname_field', true);
$first_name = get_post_meta( $post_id, 'lc_fac_staff_dir_firstname_field', true);
$middle_initial = get_post_meta( $post_id, 'lc_fac_staff_dir_minitial_field', true);

if($middle_initial != ""){
  $middle_initial = $middle_initial . ".";
}

$jobtitle_args = array('orderby' => 'name', 'order' => 'ASC', 'fields' => 'all');
$job_title = wp_get_post_terms( $post_id, 'lcdeptdir_acadfocus', $jobtitle_args );

$office_loc = get_post_meta( $post_id, 'lc_fac_staff_dir_office_location_field', true);
$phone_ext = get_post_meta( $post_id, 'lc_fac_staff_dir_phone_field', true);
//$email_addr = get_post_meta( $post_id, 'lc_fac_staff_dir_email_field', true);
$sched_appt = get_post_meta( $post_id, 'lc_fac_staff_dir_advisor_schedule_field', true);

?>
	<div class="row small-up-1 medium-up-5 fac-dir-list">
    <div class="column facdir-content" style="width:150px;"><?php echo $last_name. ", "; ?></div>
    <div class="column facdir-content" style="width:150px;"><?php echo $first_name . " " . $middle_initial; ?></div>
    <div class="column facdir-content" style="width:400px;">
    <?php
        foreach($job_title as $title){
          if($title->name <> ''){
          echo $title->name . '<br/>';
          }						
        }
    ?>
    </div>
    <?php if($office_loc <> ''){ ?>
    <div class="column facdir-content" style="width:125px;"><?php echo $office_loc; ?></div>		
  <?php
  }else{ ?>
    <div class="column facdir-content" style="width:125px;">&nbsp;</div>
  <?php
  }
  ?>
  <div class="column facdir-content">
  <?php 
          if(strlen($phone_ext) == 4){
            echo '<a href="tel:+1(440) 366-' . $phone_ext .'">(440) 366-' . $phone_ext . '</a>';
          }elseif(strlen($phone_ext) == 14){
            echo '<a href="tel:+1' . $phone_ext . '">' . $phone_ext . '</a>';  
          }else{
            echo $phone_ext;
          }
      ?>
  </div>
  <!--<div class="column facdir-content"><?php //echo '<a href="mailto:' . $email_addr . '">' . $email_addr . '</a>';?></div>-->
</div>
<!-- #post-## -->
